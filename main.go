...
func weatherForecastHandler(c echo.Context) error {
	apiKey := "cc0a364fa16d48d0941135837230311" // Gantilah dengan kunci API WeatherAPI.com Anda
	location := c.QueryParam("location")
	if location == "" {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Lokasi diperlukan"})
	}

	weather, err := fetchWeatherData(apiKey, location)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Kesalahan mengambil data cuaca"})
	}
...
func main() {
	e := echo.New()
	e.GET("/cuaca", weatherForecastHandler)

	port := 8080 // Pilih port yang ingin Anda gunakan
	e.Logger.Infof("Server berjalan di port %d...", port)
